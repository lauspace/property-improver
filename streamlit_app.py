...
with col1:
    ...
    if st.button("Pulsa para ejecutar1"):
...
with col1:
    ...
    image3 = Image.open(BytesIO(requests.get(image3_url).content))
    st.image(image3, caption="Imagen 3", use_column_width=True)
    if st.button("Pulsa para ejecutar3"):
...
with col2:
    ...
    image4 = Image.open(BytesIO(requests.get(image4_url).content))
    st.image(image4, caption="Imagen 4", use_column_width=True)
    if st.button("Pulsa para ejecutar4"):
